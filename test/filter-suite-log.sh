...
PRGNAME=$(basename "$0")
SCRIPTDIR=$(dirname "$0")
OSSFSDIR=$(cd "${SCRIPTDIR}"/.. || exit 1; pwd)
TOPDIR=$(cd "${OSSFSDIR}"/test || exit 1; pwd)
...
while read -r line; do
    ...
    if [ "${prev_line_type}" -eq 1 ]; then
        if [ "${number_type[1]}" -eq 2 ]; then
            # if passed, cut ossfs information messages
            head "-${head_line_cnt}" "${SUITELOG}" | tail "-${tail_line_cnt}" | grep -v -e '[0-9]\+\%' | grep -v -e '^ossfs: ' -a -e '\[INF\]'
        ...
        if [ "${number_type[1]}" -eq 2 ] || [ "${number_type[1]}" -eq 3 ]; then
            ...
            # this area is not from start to end, cut ossfs information messages
            head "-${head_line_cnt}" "${SUITELOG}" | tail "-${tail_line_cnt}" | grep -v -e '[0-9]\+\%' | grep -v -e '^ossfs: ' -a -e '\[INF\]'
        fi
    else
        if [ "${number_type[1]}" -eq 2 ] || [ "${number_type[1]}" -eq 3 ]; then
            ...
            # this area is normal, cut ossfs information messages
            head "-${head_line_cnt}" "${SUITELOG}" | tail "-${tail_line_cnt}" | grep -v -e '[0-9]\+\%' | grep -v -e '^ossfs: ' -a -e '\[INF\]'
        fi
    fi
...
if [ "${prev_line_type}" -eq 1 ]; then
    ...
    tail "-${tail_line_cnt}" "${SUITELOG}" | grep -v -e '[0-9]\+\%' | grep -v -e '^ossfs: ' -a -e '\[INF\]'
fi
