...
class OssDirHandle {
 public:
  ...
  int refresh_dir(std::map<estring, ObjDirent> &dirty_children);
  ...
  void get_cur_list_res(std::vector<ObjDirent> &ents) const {
    ents = cur_list_res_;
  }
 ...
 private:
  ...
  std::vector<ObjDirent> cur_list_res_;
  ...
  std::vector<ObjDirent> last_response_;
  const size_t last_response_cap_ = 100;

  std::map<estring, ObjDirent> dirty_children_;
  std::map<estring, ObjDirent>::iterator dirty_child_iter_;
