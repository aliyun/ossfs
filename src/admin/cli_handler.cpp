...
static int send_uds_request_common(pid_t pid, const std::string &command,
  ...
  std::string output;
  send_uds_request(uds_path, command, params, output);
...
int warmup_command_handler(pid_t pid, const std::string &warm_path) {
  return send_uds_request_common(pid, "warmup", warm_path);
}

int dump_task_status_command_handler(pid_t pid) {
  return send_uds_request_common(pid, "dump-status");
}
