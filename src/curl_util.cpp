...
std::string get_canonical_headers(const struct curl_slist* list, bool only_amz)
{
    ...
    for( ; list; list = list->next){
        ...
        if(only_amz && strhead.substr(0, 5) != "x-oss"){
            continue;
        }
        canonical_headers += strhead;
        canonical_headers += "\n";
...
std::string get_canonical_headers_oss(const struct curl_slist* list, bool only_oss)
{
    std::string canonical_headers;
...
bool get_canonical_resource_oss(const char* realpath, std::string& resourcepath)
{
    ...
    resourcepath = service_path + S3fsCred::GetBucket() + realpath;
    return true;    
}
